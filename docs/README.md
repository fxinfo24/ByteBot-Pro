# 📚 ByteBot-Pro Documentation

This directory contains all user-facing documentation for ByteBot-Pro.

## 📁 Documentation Files

- **[QUICK_START.md](QUICK_START.md)** - 5-minute deployment guide with 12 AI models (3 FREE)
- **[DEPLOYMENT.md](DEPLOYMENT.md)** - Advanced deployment options and troubleshooting
- **[ATLASSIAN_INTEGRATION.md](ATLASSIAN_INTEGRATION.md)** - Jira & Confluence automation setup
- **[DOCUMENTATION_INDEX.md](DOCUMENTATION_INDEX.md)** - Complete navigation guide
- **[PROJECT_ATTRIBUTION.md](PROJECT_ATTRIBUTION.md)** - Credits to original ByteBot team and our enhancements

## 🚀 Getting Started

For new users, start with the **[QUICK_START.md](QUICK_START.md)** guide.

For the main project overview, see the **[main README](../README.md)**.